...
export const searchDishesByKeyword = async (req, res) => {
    try {
        const dishes = await Dish.find({ $text: { $search: req.params.keywords } })
        return res.status(200).json({ success: true, data: dishes })
    } catch (err) {
        console.log("Error while searching dishes by keywords: ", err.message);
    }
}